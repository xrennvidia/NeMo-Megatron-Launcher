...
class NemoMegatronStage:
    ...
    def _cuda_device_max_connections(self) -> str:
        ...
        return "CUDA_DEVICE_MAX_CONNECTIONS=1" if tensor_model_parallel_size > 1 or context_parallel_size > 1 else ""
...
class Training(NeMoStage):
    ...
    def _make_hydra_override(self) -> List:
        ...
        if self.stage_cfg.model.get("ub_tp_comm_overlap", False):
            ...
            hydra_override += [
                f"'+model.ub_tp_comm_overlap_cfg={ub_cfg_name}'"
            ]
    ...
    def _get_ub_cfg_override(self) -> str:
        ...
        cfg_name = f"/tp_overlap/ub_cfg_\\${{gpu_name:}}_h{hidden_size}_tp{tp_size}_mbs{mb_size}_seqlen{seqlen}.yaml"
        return cfg_name
